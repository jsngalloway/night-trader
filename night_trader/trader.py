...
def buyAndWait(r):
    ...
    print(
        f"[{datetime.now().strftime('%H:%M:%S')}] Buying (Limit: ${round(will_pay, 2)})...",
        end="",
    )
    cleared = False
    id = buy_info["id"]
    ...
    while not cleared:
        ...
        if order_info["state"] == "filled":
            print(
                f"bought [{datetime.now().strftime('%H:%M:%S')}] (Exec: ${round(float(order_info['average_price']), 2)})"
            )
            cleared = True
        ...
        if tries > 50:
            ...
            while order_info["state"] != "canceled":
                ...
                if order_info["state"] == "filled":
                    print(
                        f"cancel failed: bought [{datetime.now().strftime('%H:%M:%S')}] (Exec: ${round(float(order_info['average_price']), 2)})"
                    )
                    cleared = True
                    return (True, float(order_info["average_price"]))
...
def sellAndWait(r):
    ...
    print(
        f"[{datetime.now().strftime('%H:%M:%S')}] Selling (Limit: ${round(will_get, 2)})...",
        end="",
    )
    cleared = False
    ...
    while not cleared:
        ...
        if order_info["state"] == "filled":
            print(
                f"[{datetime.now().strftime('%H:%M:%S')}] sold (Exec: ${round(float(order_info['average_price']), 2)})"
            )
            cleared = True
            return (True, float(order_info["average_price"]))
        ...
        if tries > 50:
            ...
            while order_info["state"] != "canceled":
                ...
                if order_info["state"] == "filled":
                    print(
                        f"cancel failed: sold [{datetime.now().strftime('%H:%M:%S')}] (Exec: ${round(float(order_info['average_price']), 2)})"
                    )
                    cleared = True
                    return (True, float(order_info["average_price"]))
                time.sleep(50 / 1000)  # wait until the order is confirmed canceled
