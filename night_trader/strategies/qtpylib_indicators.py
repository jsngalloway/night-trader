...
def heikinashi(bars):
    ...
    bars["ha_open"] = bars["ha_close"]

    # ha open
    bars.iloc[0]["ha_open"] = (bars.iloc[0]["open"] + bars.iloc[0]["close"]) / 2
    for i in range(1, len(bars)):
        bars.iloc[i]["ha_open"] = (
            bars.iloc[i - 1]["ha_open"] + bars.iloc[i - 1]["ha_close"]
        ) / 2
...
def vwap(bars):
    ...
    return pd.Series(
        index=bars.index, data=np.cumsum(volume * typical) / np.cumsum(volume)
    )
